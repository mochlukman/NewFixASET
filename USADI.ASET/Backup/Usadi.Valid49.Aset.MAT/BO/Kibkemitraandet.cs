...
namespace Usadi.Valid49.BO
{
  ...
  [Serializable]
  public class KibkemitraandetControl : BaseDataControlAsetMAT, IDataControlUIEntry, IHasJSScript
  {
    #region Properties
    ...
    public new void SetFilterKey(BaseBO bo)
    {
      ...
      else if (typeof(KibkemitraanControl).IsInstanceOfType(bo))
      {
        Unitkey = ((KibkemitraanControl)bo).Unitkey;
        Asetkey = ((KibkemitraanControl)bo).Asetkey;
        Keyinv = ((KibkemitraanControl)bo).Keyinv;
      }
    }
    ...
    {
      string sql = @"
        exec [dbo].[REGISTER_ASETKEMITRAANDET]
		    @UNITKEY = N'{0}',
		    @ASETKEY = N'{1}',
		    @KEYINV = N'{2}'
      ";
