...
namespace Usadi.Valid49.BO
{
  ...
  public class DlgRptKIBGControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public string GetURLReport(int n)
    {
      ...
      if(n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.KIBG.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "KIBG_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.KIBG.rpt", CS, Params);
      } 
      return pdfurl;
    }
