    ...
    if (GlobalAsp.CekSessionGrid(Page))
    {
      ...
      if (string.IsNullOrEmpty(Request["back"]) && string.IsNullOrEmpty(Request["passdc"]))
      {
        try
        {
          if (!Page.IsPostBack)
          {
            Usadi.Valid49.BO.ProfilunitControl temp = new Usadi.Valid49.BO.ProfilunitControl();
            IDataControl temp2 = UtilityBO.Create("Usadi.Valid49.BO.ProfilunitControl, Usadi.Valid49.MA ");
